...
function getFinalSpeed(gen, pokemon, field, side) {
	...
	else if (pokemon.hasAbility('Pickpocket', 'Rattled') && field.hasTerrain('Darkness 1', 'Darkness 2', 'Darkness 3')) {
		speedMods.push(6144);
	}
	else if (pokemon.hasAbility('Dark Surge') && field.hasTerrain('Darkness 3')) {
		speedMods.push(6144);
	}
	else if (pokemon.hasAbility('Forecast') && field.hasTerrain('Cloud')) {
		speedMods.push(6144);
	}
...
function checkSeedBoost(pokemon, field) {
    ...
    if (field.terrain && pokemon.item.includes('Seed')) {
        if (pokemon.hasItem('Telluric Seed')) {
			...
			else if (field.hasTerrain('Crowd')) {
                pokemon.boosts.atk = pokemon.hasAbility('Contrary')
                    ? Math.max(-6, pokemon.boosts.spa - 1)
                    : Math.min(6, pokemon.boosts.spa + 1);
			}
